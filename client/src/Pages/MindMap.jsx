...
function EditableNode({ id, data, selected, onContextMenu }) {
  let color = NODE_COLORS[data.type] || NODE_COLORS[data.level] || NODE_COLORS[0];
  if (data.bg) color = { bg: data.bg, text: data.text || '#222' };
...
function MindMapCore() {
  ...
  useEffect(() => {
    ...
    console.log("🔁 Mind map nodes and edges updated from Zustand!");
  }, [mindMap]);
  ...
  const renderContextMenu = () => {
    ...
    return (
      ...
      >
        <button onClick={handleEdit} style={menuBtnStyle}>✏️ Edit</button>
        <div style={{ fontSize: 13, margin: '4px 0 2px 0', color: '#888' }}>Change color:</div>
        ...
        <button onClick={handleDelete} style={{ ...menuBtnStyle, color: '#d32f2f' }}>🗑️ Delete</button>
      </div>
    );
  };
  ...
  return (
    <div className="w-full flex relative" style={{ minHeight: '100vh' }}>
      <div className="flex-grow h-[85vh]">
        <ReactFlow
          ...
          nodeTypes={{
            editable: (props) => (
              <EditableNode
                {...props}
                onContextMenu={e => onNodeContextMenu(e, props)}
              />
            ),
          }}
          fitView
        ...
        >
          <MiniMap
            nodeColor={n => n.data?.bg || '#1976d2'}
            nodeStrokeColor={n => n.selected ? '#1976d2' : '#fff'}
            nodeBorderRadius={8}
          />
